...
Статус: финальная проектная политика; меры, зависящие от реальной установки, остаются TODO и не
считаются фактически проверенными.
...
На этапе 12 `entityId` карточки всегда проверяется через серверный request scope. Историческая
FormVersion возвращается только при подтверждённом доступе к заявке и совпадении сохранённого
`formVersionId`; SurveyJS работает в режиме `display`, команды изменения `fieldValues` нет.
INTERNAL-комментарии и внутренние события отфильтровывает сервер. Статус, назначение и комментарий
используют атомарный `expectedVersion`; скрытие операторских элементов остаётся только UX.

На этапе 13 отсутствие `userId` означает профиль субъекта текущей сессии. Чужой `userId` сервер
разрешает только SYSTEM_ADMIN. `usersUpdate` имеет два непересекающихся allowlist: self — только
`language/timezone`, admin — основные данные и членство; `id/login` запрещены всегда. Защищённые
поля от обычного пользователя возвращают `FORBIDDEN`, а не игнорируются. Три списка заявок получают
группы и область доступа из доверенных серверных источников и выполняют пагинацию до возврата DTO.
...
До проверки реального Naumen FileAdapter не объявляется рабочим. На этапе 15 отсутствие данных
текущей установки подтверждено локальной проверкой конфигурации и документации проекта;
`FileFunctions` работает только в fail-closed режиме. Чек-лист интеграции приведён в
`docs/naumen-files.md`.
...
- Bootstrap 5.3.3 и QUnit 2.24.1 зафиксированы в CDN URL; Bootstrap использует SRI. SurveyJS сейчас загружается с unpkg без фиксированной версии и SRI. До production все зависимости необходимо зафиксировать, проверить по лицензии/хешу и разместить same-origin либо использовать утверждённый CDN — TODO(DEPENDENCIES).
